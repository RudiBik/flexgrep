  ...
  : mPattern(pattern)
{
    ...
    if (pos != string::npos)
        throw invalid_argument(
          "Multiple adjacent stars are not allowed in pattern: " + mPattern);
}

bool
Wildcard::match(const string& text) const
{
    if (mPattern.empty())
        return false;
...
bool
Wildcard::subMatch(StringIterator sp,
                   StringIterator ep,
                   StringIterator st,
                   StringIterator et) const
{
    ...
    if ((*tokenPos) == '?') {
        ...
        // subMatchStar handles all cases where a star before the upcoming
        // pattern is present
    } else if ((*tokenPos) == '*') {
        ...
        throw logic_error(
          "The following token isn't handled by Wildcard::subMatch: " +
          to_string((*tokenPos)));
    }
...
bool
Wildcard::subMatchStar(StringIterator sp,
                       StringIterator ep,
                       StringIterator st,
                       StringIterator et) const
{
    ...
    if (tokenPos == ep) {
        ...
        throw logic_error(
          "The following token isn't handled by Wildcard::subMatchStar: " +
          to_string((*tokenPos)));
    }
